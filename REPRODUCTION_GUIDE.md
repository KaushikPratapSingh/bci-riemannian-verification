...
>   Both now have one.
>
> **Changelog (post-submission peer-review follow-up):**
> - §3.1 updated: the paper's originally-stated causes for the `-3.42` 1/f
>   slope (causal-filter group delay) and the `-33.0%` beta ERD/ERS
>   direction error (AR(2) cross-frequency coupling) are both corrected —
>   see the inline note in §3.1 for the real causes and fixes.
> - A related open problem (noise-floor tuning trades off against
>   Riemannian state-separability) is disclosed in §3.1 and in
>   `SCRIPT_INDEX.md`, with full detail in
>   `OPEN_PROBLEM_noise_floor_tradeoff.md`.
...
> **⚠️ Post-submission update (paper Section 10, Appendix B items 9–11):**
> the numbers above are the paper's original headline session result and are
> preserved exactly as published. Two things about the *diagnosis* behind
> them have since changed:
> - The `-3.42` slope is **not** a causal-filter group-delay artefact as
>   originally stated — the real cause is the slope-fit window (4–40 Hz)
>   extending into the bandpass filter's own 45 Hz corner. Fix: narrow the
>   fit to 4–23 Hz in `evaluate_session_v2.py`.
> - The `-33.0%` beta direction error is **not** caused by unspecified AR(2)
>   cross-frequency coupling — the real cause is the alpha oscillator's
>   damping (`r=0.93`) letting ~19% of its own power bleed into the beta
>   band. Fix: sharpen to `r=0.99` in `virtual_brain_v4_lsl.py`.
>
> Applying both fixes together improves the slope and beta direction, but
> raising the background noise floor enough to fully close the remaining
> slope gap erodes the REST/COG/MOTOR distance separability shown above
> below the paper's own `>1.0` gate — an **open problem**, not yet a closed
> fix. See `SCRIPT_INDEX.md`'s open-problem note and
> `OPEN_PROBLEM_noise_floor_tradeoff.md` for the full investigation before
> using `virtual_brain_v4_lsl.py` or `evaluate_session_v2.py` to generate
> new numbers for any purpose beyond reproducing the exact session above.
...
├── requirements.txt
├── paper/
│   ├── Singh_2026_BCI_Verification.pdf
│   ├── Singh_2026_BCI_Verification.tex
│   └── figures/
├── section10_open_problem_investigation/
│   ├── README.md
