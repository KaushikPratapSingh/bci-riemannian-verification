...
🐛 **Bug Registry:** Appendix B of the paper (11 entries — algorithmic bugs, a duplicate-script finding, and two post-submission peer-review corrections)
🔓 **Open Problem:** [`OPEN_PROBLEM_noise_floor_tradeoff.md`](./OPEN_PROBLEM_noise_floor_tradeoff.md) — a disclosed, unresolved trade-off between the 1/f slope and Riemannian state-separability gates, documented for follow-up work
🔬 **Section 10 Investigation:** [`section10_open_problem_investigation/`](./section10_open_problem_investigation/) — every script, patch, and real live-run evidence file (npz/CSV/PNG) behind the open problem above, including the pink-noise-floor and slope-fit-window (`fmax`) experiments
...
.
├── paper/
│   ├── Singh_2026_BCI_Verification.pdf   # The manuscript
│   ├── Singh_2026_BCI_Verification.tex   # LaTeX source
│   └── figures/                          # All 8 paper figures (LaTeX-ready copies)
├── figures/                          # All 8 paper figures, source-traceable to scripts below
├── section10_open_problem_investigation/  # Scripts, patches, and real evidence behind the disclosed open problem
├── OPEN_PROBLEM_noise_floor_tradeoff.md   # Full narrative writeup of the open problem
...
| `heuristic_sensitivity.py` | Synthetic FastICA-vs-SOBI benchmark under extreme noise (SNR = -18.68 dB) — the source of the +19.99 dB gain and 51.6%/43.2% correlation figures (paper Section 4.2-4.3). Not a sensitivity analysis of gate thresholds, despite the filename. |
...
9. Live-session 1/f slope steepening misdiagnosed as causal-filter group-delay distortion — real cause was the slope-fit window extending into the filter's own 45 Hz rolloff. Fixed: narrowed 4-40 Hz fit to 4-23 Hz.
10. Beta ERD/ERS direction anomaly misdiagnosed as unspecified AR(2) cross-frequency coupling — real cause was the alpha oscillator's damping (r=0.93) letting ~19% of its own power bleed into the beta band. Fixed: sharpened to r=0.99.
11. `virtual_brain_v4_lsl.py`'s subject-profile and noise-generator random streams were seeded from wall-clock time, confounding every live parameter comparison with a freshly randomized simulated subject. Fixed: seeds pinned. **Open problem, not yet closed:** the noise-floor value needed to fully resolve bug #9's residual slope gap still trades off against Riemannian state-separability — see `SCRIPT_INDEX.md`, `OPEN_PROBLEM_noise_floor_tradeoff.md`, and `section10_open_problem_investigation/` (full scripts, patches, and real live-run evidence) for the complete investigation.
