"""
BCI Systems Engineering - Cohort-Scale Machine Learning Tournament (v17)
=============================================================================
Author: Kaushik Pratap Singh (Principal Investigator)

VERSION: v17 -- Introduces Subspace Riemannian Ensembling (SRE), splitting 
                the channel layout into independent Left and Right hemisphere 
                manifolds before computing localized tangent space projections.
"""
...
def main():
    ...
    print("\n" + "=" * 125)
    print(" 🚀 INITIATING COHORT-SCALE RIEMANNIAN ML TOURNAMENT (v17 - SUBSPACE RIEMANNIAN ENSEMBLING)")
    print("=" * 125)
    ...
    for sid in subject_ids:
        try:
            ...
            # Global baseline structures for fault detection & alpha weighting
            ...
            # Use global mean matrix for baseline rejection profile
            ...
            temp_act_left_covs, temp_act_right_covs, temp_act_global_covs = [], [], []
            ...
            for i, ann in enumerate(active_raw.annotations):
                ...
                # Global structural anomaly validation
            ...
            # REFINEMENT: Compute separate balanced Joint-Class Riemannian Means for EACH hemisphere subspace
            ...
            for c_glob, c_left, c_right, spec in zip(temp_act_global_covs, temp_act_left_covs, temp_act_right_covs, temp_active_spectral):
                ...
                ts_left   = project_to_tangent_space(c_left, M_joint_left)
                ts_right  = project_to_tangent_space(c_right, M_joint_right)
                
                ts_vector = np.concatenate([ts_left, ts_right, normalized_spec])
                
                sub_raw_feats.append(raw_vector)
                sub_ts_feats.append(ts_vector)
                sub_labels.append(1)
            ...
            # Separate the sub-hemisphere manifolds from the spectral tail
            # 2x2 symmetric covariance matrices have 3 distinct tangent space elements. 2 subspaces * 3 = 6 entries.
            sub_ts_riemannian = sub_ts_feats[:, :6] 
            sub_ts_spectral   = sub_ts_feats[:, 6:]
    ...
    print("\n" + "=" * 125)
    print(" 🏆 FINAL TOURNAMENT BENCHMARK COMPARISON REPORT (v17 - SUBSPACE MANIFOLDS)")
    print("=" * 125)
