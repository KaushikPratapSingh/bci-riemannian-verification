"""
BCI Systems Engineering - Cohort-Scale Machine Learning Tournament (v30)
=============================================================================
Author: Kaushik Pratap Singh (Principal Investigator)

VERSION: v30 -- Includes Fixed Dynamic Aggregation and Visualization Engine.
"""
...
try:
    ...
    print("Error: 'matplotlib' and 'seaborn' are required for visuals. Run: pip install matplotlib seaborn")
    sys.exit(1)
...
def main():
    ...
    print("\n" + "=" * 125)
    print(" 🚀 INITIATING COHORT-SCALE RIEMANNIAN ML TOURNAMENT (v30 - TASK-CONDITIONAL MANIFOLD CENTERING)")
    print("=" * 125)
    ...
    for test_sid in unique_groups:
        ...
        if subject_trial_counts[test_sid] >= MIN_CLEAN_TRIALS:
            ...
            svm_tsa_clean.append(fold_svm_tsa)
    ...
    # ==============================================================================
    # DYNAMIC LAYOUT TEXT REPORT ALIGNMENT
    # ==============================================================================
    ...
    mean_svm_tsa_all  = np.mean(svm_tsa_accs) * 100
    ...
    mean_svm_tsa_clean = np.mean(svm_tsa_clean) * 100

    print("\n" + "=" * 125)
    print(" 🏆 FINAL TOURNAMENT BENCHMARK COMPARISON REPORT (v30 - HONEST COHORT MEANS)")
    print("=" * 125)
    print(f" Subjects processed: {n_all} total  |  {n_clean} clean (>= {MIN_CLEAN_TRIALS} trials)  |  {n_isolated} isolated (flagged below)")
    if isolated_subjects:
        print(f" Isolated subjects: {['S'+str(s).zfill(2) for s in sorted(isolated_subjects)]}")
    print("=" * 125)
    ...
    print(f" {'SVM Kernel RBF + Nested CV (Approach C: Global Scaling TSA)':<55} | {mean_svm_tsa_all:18.2f}%       | {mean_svm_tsa_clean:16.2f}%")
    print("=" * 125)
    print(f"\n NOTE: 'All subjects' is the primary reported number -- every processed subject contributes equally.")
    
    # ==============================================================================
    # AUTOMATED SYSTEM VISUALIZATION ENGINE
    # ==============================================================================
    print("\n📊 Generating system benchmark visualization layouts...")
    fig, axes = plt.subplots(1, 2, figsize=(16, 7))
    
    # Plot 1: Sub-cohort performance breakdown
    categories = ['RF Raw', 'RF TSA', 'SVM Raw', 'SVM TSA']
    all_means = [mean_rf_raw_all, mean_rf_tsa_all, mean_svm_raw_all, mean_svm_tsa_all]
    clean_means = [mean_rf_raw_clean, mean_rf_tsa_clean, mean_svm_raw_clean, mean_svm_tsa_clean]
    ...
    # Plot 2: Detailed subject validation tracking
    subject_ticks = [f"S{str(s).zfill(2)}" for s in unique_groups]
    axes[1].plot(subject_ticks, [v*100 for v in svm_tsa_accs], marker='o', linewidth=2, color='#e67e22', label='🏆 SVM TSA (Target Method)')
    ...
    # Highlight isolated subjects directly on tracking chart
    for i, sid in enumerate(unique_groups):
        if sid in isolated_subjects:
            axes[1].scatter(subject_ticks[i], svm_tsa_accs[i]*100, color='red', s=120, zorder=5, facecolors='none', edgecolors='r', linewidths=2)
            
    axes[1].set_ylabel('Fold Accuracy (%)', fontsize=12, fontweight='bold')
    axes[1].set_title('B: Cross-Subject Validation Fold Distribution', fontsize=13, fontweight='bold', pad=15)
    ...
    plt.tight_layout()
    output_fig = "bci_tournament_performance.png"
    plt.savefig(output_fig, dpi=300)
    print(f"💾 Visual analytics plot successfully compiled and stored to disk: '{output_fig}'\n")
