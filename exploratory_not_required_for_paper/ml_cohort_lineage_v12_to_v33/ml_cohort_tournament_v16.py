"""
BCI Systems Engineering - Cohort-Scale Machine Learning Tournament (v16)
=============================================================================
Author: Kaushik Pratap Singh (Principal Investigator)

VERSION: v16 -- Introduces Ledoit-Wolf analytical covariance shrinkage and a 
                balanced Joint-Class Riemannian Mean anchor for unbiased 
...
def compute_shrunk_covariance(epoch_data):
    """Computes an optimal analytically shrunk covariance matrix via Ledoit-Wolf."""
...
def main():
    ...
    print("\n" + "=" * 125)
    print(" 🚀 INITIATING COHORT-SCALE RIEMANNIAN ML TOURNAMENT (v16 - LEDOIT-WOLF & BALANCED JC-MEAN ANCHORS)")
    print("=" * 125)
    ...
    for sid in subject_ids:
        try:
            ...
            for s in range(0, len(rest_data) - window_n, step_n):
                cov = compute_shrunk_covariance(rest_data[s:s + window_n])
            ...
            for i, ann in enumerate(active_raw.annotations):
                ...
                trial_cov = compute_shrunk_covariance(trial_clean)
                
                trial_dist = riemannian_distance(trial_cov, M_rest_init)
                if trial_dist > fault_threshold:
                    continue
                
                temp_active_covs.append(trial_cov)
                temp_active_spectral.append(extract_channel_spectral_powers(trial_clean, FS))

            trial_count = len(temp_active_covs)
            if trial_count < 2:
                continue

            rng = np.random.default_rng(sid)
            chosen_rest_indices = rng.choice(len(rest_covariances), size=trial_count, replace=False)
            
            selected_rest_covs = rest_covariances[chosen_rest_indices]
            selected_rest_spec = rest_spectral_list[chosen_rest_indices]

            # REFINEMENT: Compute a true balanced Joint-Class Riemannian Center for reference projection
            balanced_cohort_covs = np.concatenate([selected_rest_covs, np.array(temp_active_covs)], axis=0)
            M_joint = compute_riemannian_mean(balanced_cohort_covs)

            sub_raw_feats, sub_ts_feats, sub_labels = [], [], []

            # Vectorize Active Trials
            for cov, spec in zip(temp_active_covs, temp_active_spectral):
                normalized_spec = (spec - spec_mean) / spec_std
                sub_raw_feats.append(np.concatenate([vectorize_raw_covariance(cov), normalized_spec]))
                sub_ts_feats.append(np.concatenate([project_to_tangent_space(cov, M_joint), normalized_spec]))
                sub_labels.append(1)

            # Vectorize Rest Trials
            for cov, spec in zip(selected_rest_covs, selected_rest_spec):
                normalized_spec = (spec - spec_mean) / spec_std
                sub_raw_feats.append(np.concatenate([vectorize_raw_covariance(cov), normalized_spec]))
                sub_ts_feats.append(np.concatenate([project_to_tangent_space(cov, M_joint), normalized_spec]))
                sub_labels.append(0)
    ...
    print("\n" + "=" * 125)
    print(" 🏆 FINAL TOURNAMENT BENCHMARK COMPARISON REPORT (v16 - REFINED MANIFOLDS)")
    print("=" * 125)
