"""
BCI Systems Engineering - Cohort-Scale Machine Learning Tournament (v22)
=============================================================================
Author: Kaushik Pratap Singh (Principal Investigator)

VERSION: v22 -- Introduces Adaptive Rolling Riemannian Memory. Tracks session 
                non-stationarity and temporal fatigue by updating the tangent space 
                reference frame using a geodesic moving average over a rolling window 
                of active trials.
"""
...
def update_geodesic_rolling_mean(M_current, C_new, lambda_step=0.15):
    """
    Performs a first-order geodesic step projection on the SPD manifold 
    to continuously track temporal non-stationarity.
    """
    vals, vecs = np.linalg.eigh(M_current)
    vals = np.clip(vals, 1e-10, None)
    M_half = vecs @ np.diag(np.sqrt(vals)) @ vecs.T
    M_neg_half = vecs @ np.diag(1.0 / np.sqrt(vals)) @ vecs.T
    
    projected = M_neg_half @ C_new @ M_neg_half
    p_vals, p_vecs = np.linalg.eigh(projected)
    p_vals = np.clip(p_vals, 1e-10, None)
    log_proj = p_vecs @ np.diag(np.log(p_vals)) @ p_vecs.T
    
    step_tangent = lambda_step * log_proj
    t_vals, t_vecs = np.linalg.eigh(step_tangent)
    exp_tangent = t_vecs @ np.diag(np.exp(t_vals)) @ t_vecs.T
    
    M_updated = M_half @ exp_tangent @ M_half
    return M_updated
...
def main():
    ...
    print("\n" + "=" * 125)
    print(" 🚀 INITIATING COHORT-SCALE RIEMANNIAN ML TOURNAMENT (v22 - ADAPTIVE ROLLING RIEMANNIAN MEMORY)")
    print("=" * 125)
    ...
    for sid in subject_ids:
        try:
            ...
            M_rolling_global = compute_riemannian_mean(balanced_global_covs)
            M_rolling_left   = compute_riemannian_mean(balanced_left_covs)
            M_rolling_right  = compute_riemannian_mean(balanced_right_covs)

            disp_global = np.std([riemannian_distance(C, M_rolling_global) for C in balanced_global_covs])
            disp_left   = np.std([riemannian_distance(C, M_rolling_left) for C in balanced_left_covs])
            disp_right  = np.std([riemannian_distance(C, M_rolling_right) for C in balanced_right_covs])
            ...
            for c_glob, c_left, c_right, spec in zip(selected_global_rest, selected_left_rest, selected_right_rest, selected_rest_spec):
                ...
                ts_global = project_to_tangent_space(c_glob, M_rolling_global) * w_global
                ts_left   = project_to_tangent_space(c_left, M_rolling_left) * w_left
                ts_right  = project_to_tangent_space(c_right, M_rolling_right) * w_right
            ...
            # 3. Dynamic Rolling Adaptation during Active Trials processing
            for c_glob, c_left, c_right, spec in zip(temp_act_global_covs, temp_act_left_covs, temp_act_right_covs, temp_active_spectral):
                normalized_spec = (spec - spec_mean) / spec_std
                raw_vector = np.concatenate([vectorize_raw_covariance(c_glob), normalized_spec])
                
                # Execute rolling geometric memory step updates
                M_rolling_global = update_geodesic_rolling_mean(M_rolling_global, c_glob, lambda_step=0.15)
                M_rolling_left   = update_geodesic_rolling_mean(M_rolling_left, c_left, lambda_step=0.15)
                M_rolling_right  = update_geodesic_rolling_mean(M_rolling_right, c_right, lambda_step=0.15)
            ...
            print(f"  Processed Subject S{sid:02d} | Rolling Memory Tracker: ACTIVE | Alpha: {alpha_s:.3f}")
    ...
    print("\n" + "=" * 125)
    print(" 🏆 FINAL TOURNAMENT BENCHMARK COMPARISON REPORT (v22 - ADAPTIVE ROLLING RIEMANNIAN MEMORY)")
    print("=" * 125)
    print(f" {'Random Forest (Approach A: Raw)':<50} | {np.mean(rf_raw_accs)*100:18.2f}%")
    print(f" {'Random Forest (Approach C: Rolling Adaptive TSA)':<50} | {np.mean(rf_tsa_accs)*100:18.2f}%")
    print("-" * 125)
    print(f" {'SVM Kernel RBF + Nested CV (Approach A: Raw)':<50} | {np.mean(svm_raw_accs)*100:18.2f}%")
    print(f" {'SVM Kernel RBF + Nested CV (Approach C: Rolling Adaptive TSA)':<50} | {np.mean(svm_tsa_accs)*100:18.2f}%")
    print("=" * 125)
