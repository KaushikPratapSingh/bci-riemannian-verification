"""
BCI Systems Engineering - Cohort-Scale Machine Learning Tournament (v29)
=============================================================================
Author: Kaushik Pratap Singh (Principal Investigator)

VERSION: v29 -- Task-Conditional Manifold Centering.
                Abandons chronological window-based scaling. Isolates Rest (0)
                and Active (1) trial blocks per subject to compute state-specific
                centroids and dispersion profiles, ensuring cross-subject 
                features are aligned based purely on task-evoked transitions.
"""
...
def main():
    ...
    print("\n" + "=" * 125)
    print(" 🚀 INITIATING COHORT-SCALE RIEMANNIAN ML TOURNAMENT (v29 - TASK-CONDITIONAL MANIFOLD CENTERING)")
    print("=" * 125)
    ...
    for sid in subject_ids:
        try:
            ...
            # -----------------------------------------------------------------
            # CRITICAL STRUCTURAL UPDATE: TASK-CONDITIONAL MANIFOLD CENTERING
            # -----------------------------------------------------------------
            # 1. Gather all raw components natively mapped to their matching classes
            ...
            # 2. Compute true centroids across isolated operational conditions
            rest_centroid = np.mean(sub_rest_tsa_riemannian, axis=0)
            active_centroid = np.mean(sub_active_tsa_riemannian, axis=0)

            # 3. Assess internal dispersion factors completely independent of state cross-talk
            ...
            # 4. Standardize spatial fields using state-specific boundaries
            norm_rest_riem = sub_rest_tsa_riemannian / sigma_rest
            norm_active_riem = sub_active_tsa_riemannian / sigma_active

            # -----------------------------------------------------------------
            # MULTI-BLOCK GEOMETRIC ELEMENT-WISE BALANCING
            # -----------------------------------------------------------------
            ...
            # Assemble clean Rest feature matrices
            ...
            # Assemble clean Active feature matrices
    ...
    print("\n" + "=" * 125)
    print(" 🏆 FINAL TOURNAMENT BENCHMARK COMPARISON REPORT (v29 - TASK-CONDITIONAL MANIFOLD CENTERING)")
    print("=" * 125)
