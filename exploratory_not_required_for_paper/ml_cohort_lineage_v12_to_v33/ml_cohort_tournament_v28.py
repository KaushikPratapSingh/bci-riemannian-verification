"""
BCI Systems Engineering - Cohort-Scale Machine Learning Tournament (v28)
=============================================================================
Author: Kaushik Pratap Singh (Principal Investigator)

VERSION: v28 -- Dimension-Averaged Variance Matching.
                Scales individual feature blocks by their average element-wise
                variance instead of total block L2 norms. This prevents the 
                higher-dimensional Riemannian block (22-D) from being structurally 
                damped relative to the lower-dimensional Spectral block (8-D).
"""
...
def main():
    ...
    print("\n" + "=" * 125)
    print(" 🚀 INITIATING COHORT-SCALE RIEMANNIAN ML TOURNAMENT (v28 - DIMENSION-AVERAGED VARIANCE MATCHING)")
    print("=" * 125)
    ...
    for sid in subject_ids:
        try:
            ...
            # -----------------------------------------------------------------
            # STRUCTURAL CORRECTION: DIMENSION-AVERAGED VARIANCE MATCHING
            # -----------------------------------------------------------------
            riem_elements_var = np.var(normalized_riem_feats, axis=0) + 1e-8
            avg_riem_element_var = np.mean(riem_elements_var)
            ...
            spec_elements_var = np.var(raw_spec_feats, axis=0) + 1e-8
            avg_spec_element_var = np.mean(spec_elements_var)

            # Generate balance multi-block transformations
            riem_scale = 1.0 / np.sqrt(avg_riem_element_var)
            spec_scale = 1.0 / np.sqrt(avg_spec_element_var)

            for t in range(len(all_riem_feats)):
                balanced_riem = normalized_riem_feats[t] * riem_scale
                balanced_spec = raw_spec_feats[t] * spec_scale
    ...
    # Expanded boundary valves for hyperparameter resolution landscape
    svm_param_grid = {
    ...
    print("\n" + "=" * 125)
    print(" 🏆 FINAL TOURNAMENT BENCHMARK COMPARISON REPORT (v28 - DIMENSION-AVERAGED VARIANCE MATCHING)")
    print("=" * 125)
