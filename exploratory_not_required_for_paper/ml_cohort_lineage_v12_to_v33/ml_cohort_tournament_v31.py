"""
BCI Systems Engineering - Cohort-Scale Machine Learning Tournament (v31)
=============================================================================
Author: Kaushik Pratap Singh (Principal Investigator)

VERSION: v31 -- Integrates production visualization engines for automated 
                performance reporting and t-SNE manifold topology mapping.
"""
...
def main():
    ...
    print("\n" + "=" * 125)
    print(" 🚀 INITIATING COHORT-SCALE RIEMANNIAN ML TOURNAMENT (v31 - TASK-CONDITIONAL MANIFOLD CENTERING)")
    print("=" * 125)
    ...
    mean_svm_tsa_clean = np.mean(svm_raw_clean) * 100 # Keep variable logic consistent with visual outputs

    print("\n" + "=" * 125)
    print(" 🏆 FINAL TOURNAMENT BENCHMARK COMPARISON REPORT (v31 - HONEST COHORT MEANS)")
    print("=" * 125)
    ...
    print(f" {'SVM Kernel RBF + Nested CV (Approach C: Global Scaling TSA)':<55} | {mean_svm_tsa_all:18.2f}%       | {np.mean(svm_tsa_clean)*100:16.2f}%")
    print("=" * 125)
    ...
    clean_means = [mean_rf_raw_clean, mean_rf_tsa_clean, mean_svm_raw_clean, np.mean(svm_tsa_clean)*100]
    ...
    try:
        # Initializing without the legacy 'n_iter' argument to support modern scikit-learn interfaces natively
