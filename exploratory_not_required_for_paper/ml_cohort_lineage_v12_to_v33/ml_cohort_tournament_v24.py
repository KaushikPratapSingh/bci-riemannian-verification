"""
BCI Systems Engineering - Cohort-Scale Machine Learning Tournament (v24)
=============================================================================
Author: Kaushik Pratap Singh (Principal Investigator)

VERSION: v24 -- Rigid Baseline Anchoring with Global Magnitude Scaling.
                Reverts element-wise micro-z-scoring to a true multi-dimensional
                global scalar divisor. This stabilizes session amplitude drift 
                and cognitive fatigue without warping the directional geometric 
                heading of the Riemannian tangent features.
"""
...
def main():
    ...
    print("\n" + "=" * 125)
    print(" 🚀 INITIATING COHORT-SCALE RIEMANNIAN ML TOURNAMENT (v24 - GLOBAL SCALAR MAGNITUDE SCALING)")
    print("=" * 125)
    ...
    for sid in subject_ids:
        try:
            ...
            # Rigid baseline coordinate anchors (v21 Legacy Architecture)
            ...
            # -----------------------------------------------------------------
            # TRUE RIEMANNIAN GLOBAL SCALING ENGINE (v24 Dynamic Implementation)
            # -----------------------------------------------------------------
            ...
            for t in range(len(all_riem_feats)):
                ...
                # 1. Compute the local multidimensional window center
                local_centroid = np.mean(window_feats, axis=0)
                
                # 2. Compute scalar global dispersion (L2 distance from center)
                distances = [np.linalg.norm(v - local_centroid) for v in window_feats]
                global_sigma = np.mean(distances) + 1e-8
                
                # 3. Apply uniform gain scale (preserves spatial direction completely)
    ...
    print("\n" + "=" * 125)
    print(" 🏆 FINAL TOURNAMENT BENCHMARK COMPARISON REPORT (v24 - GLOBAL SCALAR MAGNITUDE SCALING)")
    print("=" * 125)
