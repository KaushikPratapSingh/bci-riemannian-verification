"""
BCI Systems Engineering - Cohort-Scale Machine Learning Tournament (v23)
=============================================================================
Author: Kaushik Pratap Singh (Principal Investigator)

VERSION: v23 -- Rigid Baseline Anchoring with Rolling Variance Normalization.
                Reverts to the stable resting mean reference coordinates of v21 
                to preserve cross-subject alignment, but introduces a rolling 
                variance engine to scale feature magnitudes against session drift.
"""
...
def main():
    ...
    print("\n" + "=" * 125)
    print(" 🚀 INITIATING COHORT-SCALE RIEMANNIAN ML TOURNAMENT (v23 - ROLLING VARIANCE NORMALIZATION)")
    print("=" * 125)
    ...
    for sid in subject_ids:
        try:
            ...
            # Rigid baseline means fixed entirely from the resting run (v21 legacy)
            ...
            # Re-verify geometry weighting from balanced segments
            ...
            # Project Rest Trials to rigid baseline space
            ...
            # Project Active Trials to rigid baseline space
            ...
            # -----------------------------------------------------------------
            # ROLLING VARIANCE NORMALIZATION ENGINE (v23 Core Upgrade)
            # -----------------------------------------------------------------
            ...
            for t in range(len(all_riem_feats)):
                ...
                local_mean = np.mean(window_feats, axis=0)
                local_std = np.std(window_feats, axis=0) + 1e-8
                
                # Rescale current feature magnitude relative to dynamic local window
                normalized_riem_feats[t] = (all_riem_feats[t] - local_mean) / local_std

            for t in range(len(all_riem_feats)):
                norm_spec = (all_spectral_feats[t] - spec_mean) / spec_std
                combined_vector = np.concatenate([normalized_riem_feats[t], norm_spec])
            ...
            print(f"  Processed Subject S{sid:02d} | Variance Scaling Engine: ON | Alpha: {alpha_s:.3f}")
    ...
    print("\n" + "=" * 125)
    print(" 🏆 FINAL TOURNAMENT BENCHMARK COMPARISON REPORT (v23 - ROLLING VARIANCE NORMALIZATION)")
    print("=" * 125)
    print(f" {'Random Forest (Approach A: Raw)':<50} | {np.mean(rf_raw_accs)*100:18.2f}%")
    print(f" {'Random Forest (Approach C: Rolling Scaling TSA)':<50} | {np.mean(rf_tsa_accs)*100:18.2f}%")
    print("-" * 125)
    print(f" {'SVM Kernel RBF + Nested CV (Approach A: Raw)':<50} | {np.mean(svm_raw_accs)*100:18.2f}%")
    print(f" {'SVM Kernel RBF + Nested CV (Approach C: Rolling Scaling TSA)':<50} | {np.mean(svm_tsa_accs)*100:18.2f}%")
    print("=" * 125)
