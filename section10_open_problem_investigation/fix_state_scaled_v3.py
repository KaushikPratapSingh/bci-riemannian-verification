...
FS = 250
N_SAMPLES = 30000  # reduced from 60000
...
def riemannian_mean_fast(covs, n_iter=15, tol=1e-6):
    covs = np.array([c for c in covs if np.all(np.linalg.eigvalsh(c) > 0)])
    M = covs.mean(axis=0)
    for _ in range(n_iter):
        ...
        inner = np.einsum('ij,njk,kl->nil', M_inv_half, covs, M_inv_half)
        # batch logm via eigendecomposition (all inner are SPD)
        S_sum = np.zeros_like(M)
        for mat in inner:
            w, v = np.linalg.eigh(mat)
            w = np.clip(w, 1e-12, None)
            S_sum += v @ np.diag(np.log(w)) @ v.T
        S_mean = S_sum/len(covs)
        if np.linalg.norm(S_mean,'fro') < tol: break
        M = M_half @ (np.linalg.eigh(S_mean)[1] @ np.diag(np.exp(np.linalg.eigh(S_mean)[0])) @ np.linalg.eigh(S_mean)[1].T) @ M_half
        M = 0.5*(M+M.T)
    return M
...
def evaluate_config(rest_std, cog_std, motor_std, seed_base=300, window_step=150):
    b, a = butter_bandpass()
    eeg_rest = generate_state("REST", rest_std, seed_base+1)
    eeg_cog  = generate_state("COGNITIVE", cog_std, seed_base+2)
    eeg_motor= generate_state("MOTOR", motor_std, seed_base+3)
    ch1_f = lfilter(b, a, eeg_rest[0])
    f, psd = welch(ch1_f, fs=FS, nperseg=1024)
    slope, r2 = slope_fit(f, psd, 4, 23)
    def windowed_covs(eeg):
        return [spd_cov(eeg[:, end-500:end].T) for end in range(500, N_SAMPLES+1, window_step)]
    rest_mean = riemannian_mean_fast(windowed_covs(eeg_rest))
    cog_mean = riemannian_mean_fast(windowed_covs(eeg_cog))
    motor_mean = riemannian_mean_fast(windowed_covs(eeg_motor))
...
for rest_std, cog_std, motor_std in [
    (8, 12, 18), (12, 16, 24), (20, 25, 32), (30, 35, 42),
]:
    slope, r2, d_rc, d_rm, d_cm = evaluate_config(rest_std, cog_std, motor_std)
...
print()
print("=== Round 3: widen REST-COG gap specifically ===")
for rest_std, cog_std, motor_std in [
    (15, 30, 45), (10, 30, 45), (20, 38, 48), (10, 35, 50),
    (15, 35, 48), (8, 30, 45), (12, 32, 46), (15, 32, 42),
]:
    slope, r2, d_rc, d_rm, d_cm = evaluate_config(rest_std, cog_std, motor_std)
    slope_ok = -2.5 <= slope <= -1.0
    all_ok = slope_ok and d_rc>1 and d_rm>1 and d_cm>1
