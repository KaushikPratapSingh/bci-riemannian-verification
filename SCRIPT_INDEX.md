...
**A known ambiguity, disclosed rather than resolved:** `REPRODUCTION_GUIDE.md`
§4.3 itself flags that it's unclear whether `heuristic_sensitivity.py`
(in both `stage1/` and `stage4/`) or `swelling_reproducibility_sweep.py`
(in `stage4/`) is the canonical script behind the 20-seed swelling
reproducibility sweep and its retraction (paper §9, finding #3). Both are
kept in the repo. Confirm which one you're citing before relying on either
for a submission claim — see the guide's own wording at §4.3.
...
4. `stage4_stew_crossvalidation/`: `process_stew_benchmark.py` → `run_route_a_evaluation.py` → (swelling sweep — see ambiguity note above)
