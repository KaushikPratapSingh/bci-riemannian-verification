...
If you're trying to reproduce a specific claim in `paper/Singh_2026_BCI_Verification.pdf`,
...
## ⚠️ Open problem: 1/f slope vs. Riemannian separability trade-off (Section 10)

A post-submission peer-review recommendation led to an extended investigation
covering `virtual_brain_v4_lsl.py` and `evaluate_session_v2.py`. Two
originally-stated causes in the paper were found to be **wrong** and are
now corrected (see Appendix B, items 9-10 of the paper):

- The live-session $1/f$ slope steepening was **not** caused by causal-filter
  group delay. Real cause: the slope-fit window (4-40 Hz) extended too close
  to the bandpass filter's own 45 Hz corner. **Fixed**: narrowed to 4-23 Hz.
- The beta ERD/ERS direction anomaly was **not** caused by unspecified AR(2)
  cross-frequency coupling. Real cause: the alpha oscillator's damping
  (`r=0.93`) let ~19% of its own power bleed into the beta band. **Fixed**:
  sharpened to `r=0.99`.

A third issue remains **open**: raising the simulator's broadband noise
floor enough to fully correct the residual slope, by any single global
value tested, eroded Riemannian state-separability below the paper's own
`>1.0` geodesic-distance gate. A literature-grounded, state-dependent noise
floor (REST < COGNITIVE < MOTOR, motivated by real-EEG evidence that
aperiodic power scales with arousal) achieved a full pass once, but a
subsequently-identified confound — `virtual_brain_v4_lsl.py`'s subject
profile and noise generators were seeded from wall-clock time, so every
comparison across parameter values was also comparing across a fresh,
randomly different simulated subject — means that pass cannot yet be
trusted as a validated fix. **This confound is now fixed** (seeds pinned),
but the specific noise-floor configuration that reliably satisfies both
gates has not yet been established.

Full technical detail — the complete parameter sweep across seven tested
configurations, the offline verification methodology (with code), the exact
RNG confound diagnosis, and four concrete next steps for follow-up work —
is in `OPEN_PROBLEM_noise_floor_tradeoff.md` at the repo root, with every
underlying script, patch, and real live-run evidence file (npz/CSV/PNG from
the actual pink_std and fmax experiments) in
`section10_open_problem_investigation/` (see that folder's own README for
an index). This is disclosed in the paper's Limitations section (Section 10)
and Appendix B (item 11) rather than presented as a closed fix, consistent
with this project's verification-first, disclosure-over-omission principle.
...
| `stage3_closed_loop_simulation/` | `virtual_brain_v4_lsl.py`, `calibration_orchestrator_v2.py`, `realtime_inference_engine_v2_lsl.py`, `evaluate_session_v2.py` | Fig. 7, §6.3 | Session 1784410860 — SQI, Cohen's d, geodesics, ERD/ERS. **Note:** `virtual_brain_v4_lsl.py` and `evaluate_session_v2.py` in this folder predate the Section 10 open-problem investigation (alpha damping, slope fit window, RNG seeding). See the open-problem note above before citing new numbers from these scripts. |
...
- **`live_deployment_stack/`** — a parallel set of scripts for a real-time,
  ...
  a Flask-style `app_backend.py`, etc.). None of these are cited in the
  paper. They represent exploratory work toward a future
...
before citing those figures). `OPEN_PROBLEM_noise_floor_tradeoff.md` and
`section10_open_problem_investigation/` document a still-open verification
question separately from the load-bearing reproduction chain above — see
that folder's own README for its internal index.
